---
title: "Analysis of ESPRESSO data (Stable 06.01.2016)"
...
file.path <- c("/Users/josesho/Desktop/scratch/ESPRESSO/R15A04-all")
# Local file directory path name where data files are stored.

setwd(file.path)
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE, fig.path = paste(file.path, "/Figures/", sep = "") )
  ...
  if ( length( levels(summary.per.fly[, "contrast.groups.noFoodType"]) ) > 1 ) {
    ...
    if (m == "Average Feed Size (nl)") {
      d <- dunn.test(temp.rawdata[, plot.properties[[m]][1] ], 
                      temp.rawdata[, "contrast.groups.noFoodType"], method ="bh")
    } else {
    ...
    if ( length(levels(rawdata.all[, "FoodIdx"])) > 1 ) {
      ...
      if (m == "Average Feed Size (nl)") {
        d <- dunn.test(temp.rawdata[, plot.properties[[m]][1] ], 
                       temp.rawdata[, "contrast.groups.withFoodType"], method ="bh")
      } else {
        d <- dunn.test(summary.per.fly[, plot.properties[[m]][1] ], 
