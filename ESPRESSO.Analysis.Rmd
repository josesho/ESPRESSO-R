...
```{r TimeCoursePerContrastGroupWithFood}
...
if ( length(levels(rawdata.all[, "FoodIdx"])) > 1 ) {
  ...
  # Set order of levels of combined contrast groups, according to that dictated in `reference_levels.txt`.
  bin.per.contrastGroup.withFood[, "contrast.groups.withFoodType"] <- factor(bin.per.contrastGroup.withFood[, "contrast.groups.withFoodType"], 
                                                                             levels = unique(reflevels.combis$contrast.groups.withFoodType))
  
  bin.per.contrastGroup.withFood[, "contrast.groups.noFoodType"] <-  factor(bin.per.contrastGroup.withFood[, "contrast.groups.noFoodType"], 
                                                                            levels = unique(reflevels.combis$contrast.groups.noFoodType))
...
```{r TimeCoursePerContrastGroupNoFood}
n <- paste("bin.", as.character(bw/60), ".min", sep = "")
...
if ( length(levels(rawdata.all[, "FoodIdx"])) > 1) {
  ...
  temp.df <- data.frame( apply(summary.per.contrastGroup.withFood, 2, combn, m = 2) )
  for (i in (length(contrast.columns) + 3) : (ncol(temp.df)) ) {
  ...
  for ( i in seq(from = 1, to = nrow(temp.df), by = 2) ) {
    ...
    feed.count.escalc$comparison <-  paste(temp.df$contrast.groups.withFoodType[i], " vs ", "\t", temp.df$contrast.groups.withFoodType[i + 1], sep = "")
    ...
    time.to.first.feed.escalc$comparison <-  paste(temp.df$contrast.groups.withFoodType[i], " vs ", "\t", temp.df$contrast.groups.withFoodType[i + 1], sep = "")
    ...
    vol.intake.nanoliter.escalc$comparison <-  paste(temp.df$contrast.groups.withFoodType[i], " vs ", "\t", temp.df$contrast.groups.withFoodType[i + 1], sep = "")
    ...
    vol.per.feed.per.fly.nanoliter.escalc$comparison <-  paste(temp.df$contrast.groups.withFoodType[i], " vs ", "\t", temp.df$contrast.groups.withFoodType[i + 1], sep = "")
    ...
    # Volume per Feed 
    vol.per.feed.nanoliter.escalc <- escalc(data = temp.df, measure = "SMD", vtype = "UB", 
                                                    m1i = mean.vol.per.feed.nanoliter[i + 1], 
                                                    m2i = mean.vol.per.feed.nanoliter[i], 
                                                    n1i = sum.feed.count[i + 1], # NOTE THE CHANGE FROM TOTAL FLY COUNT TO SUM FEED COUNT.
                                                    n2i = sum.feed.count[i], # NOTE THE CHANGE FROM TOTAL FLY COUNT TO SUM FEED COUNT.
                                                    sd1i = sd.vol.per.feed.nanoliter[i + 1], 
                                                    sd2i = sd.vol.per.feed.nanoliter[i], append = FALSE)
    vol.per.feed.nanoliter.escalc <- summary.escalc(vol.per.feed.nanoliter.escalc)
    vol.per.feed.nanoliter.escalc$comparison <-  paste(temp.df$contrast.groups.withFoodType[i], " vs ", "\t", temp.df$contrast.groups.withFoodType[i + 1], sep = "")
    colnames(vol.per.feed.nanoliter.escalc) <- c("average.vol.per.feed.nanoliter.g", 
                                                 "average.vol.per.feed.nanoliter.var", 
                                                 "average.vol.per.feed.nanoliter.sei", 
                                                 "average.vol.per.feed.nanoliter.zi", 
                                                 "average.vol.per.feed.nanoliter.ci.lb",
                                                 "average.vol.per.feed.nanoliter.ci.ub", 
                                                 "comparison")
    ...
    mean.feeding.speed.per.fly.nl.per.s.escalc$comparison <-  paste(temp.df$contrast.groups.withFoodType[i], " vs ", "\t", temp.df$contrast.groups.withFoodType[i + 1], sep = "")
    ...
    proportion.of.flies.feeding.escalc$comparison <-  paste(temp.df$contrast.groups.withFoodType[i], " vs ", "\t", temp.df$contrast.groups.withFoodType[i + 1], sep = "")
    ...
    feed.duration.s.escalc$reference.contrast.group <- unite_(temp.df[i,], "reference.contrast.group", contrast.columns, sep = "\n")[, "reference.contrast.group"]
    feed.duration.s.escalc$tested.contrast.group <- unite_(temp.df[i + 1,], "tested.contrast.group", contrast.columns, sep = "\n")[, "tested.contrast.group"]
    feed.duration.s.escalc$comparison <-  paste(temp.df$contrast.groups.withFoodType[i], " vs ", "\t", temp.df$contrast.groups.withFoodType[i + 1], sep = "")
    ...
    x <- full_join(feed.duration.s.escalc, 
                   full_join(vol.per.feed.per.fly.nanoliter.escalc, 
                             full_join(vol.per.feed.nanoliter.escalc,
    ...
    summary.escalc.withFood[[paste(x$comparison)]] <- x[,c(7:15, 1:6, 16:length(x))]
  }
  ...
  # Make sure contrast.group columns are factors with appropriate factor levels and order.
  for (c in c("reference.contrast.group", "tested.contrast.group")) {
    hedges.g.withFood[, c] <- factor( hedges.g.withFood[, c],
                                      levels = unique(reflevels.combis$contrast.groups.withFoodType) )
  }
    
  # Make sure columns that should be numeric, are.
  for (colnum in 3:ncol(hedges.g.withFood)) {
    hedges.g.withFood[, colnum] <- as.numeric( as.character(hedges.g.withFood[, colnum]) )
  }
  ...
  write.csv(hedges.g.withFood, file = paste(output.path, "/hedges.g.withFood.csv", sep = ""), row.names = FALSE)
}
...
# Reformats temp.df so that all pairwise combinations are in paired rows. 
# See https://www.biostars.org/p/3694/
temp.df <- data.frame( apply(summary.per.contrastGroup.noFood, 2, combn, m = 2) )
for (i in (length(contrast.columns.noFoodType) + 2) : ncol(temp.df) ) {
  temp.df[, i] <- as.numeric( as.character(temp.df[, i]) )
}
...
for ( i in seq(from = 1, to = nrow(temp.df), by = 2) )  {
  ...
  feed.count.escalc <- escalc(data = temp.df, measure = "SMD", vtype = "UB", 
                              ...
                              sd2i = sd.total.feed.count.per.fly[i], append = FALSE)
  feed.count.escalc <- summary.escalc(feed.count.escalc)
  feed.count.escalc$comparison <- paste(temp.df$contrast.groups.noFoodType[i], " vs ", temp.df$contrast.groups.noFoodType[i + 1], sep = "")
  colnames(feed.count.escalc) <- c("average.total.feed.count.per.fly.g", 
                                   "average.total.feed.count.per.fly.var", 
                                   "average.total.feed.count.per.fly.sei", 
                                   "average.total.feed.count.per.fly.zi", 
  ...
  time.to.first.feed.escalc <- escalc(data = temp.df, measure = "SMD", vtype = "UB", 
                                      ...
                                      sd2i = sd.time.to.first.feed.s[i], append = FALSE)
  time.to.first.feed.escalc <- summary.escalc(time.to.first.feed.escalc)
  time.to.first.feed.escalc$comparison <- paste(temp.df$contrast.groups.noFoodType[i], " vs ", temp.df$contrast.groups.noFoodType[i + 1], sep = "")
  colnames(time.to.first.feed.escalc) <- c("average.time.first.feed.per.fly.g", 
                                           "average.time.first.feed.per.fly.var", 
                                           "average.time.first.feed.per.fly.sei", 
                                           "average.time.first.feed.per.fly.zi", 
  ...
  vol.intake.nanoliter.escalc <- escalc(data = temp.df, measure = "SMD", vtype = "UB", 
                                        ...
                                        sd2i = sd.mean.total.vol.intake.nanoliter[i], append = FALSE)
  vol.intake.nanoliter.escalc <- summary.escalc(vol.intake.nanoliter.escalc)
  vol.intake.nanoliter.escalc$comparison <- paste(temp.df$contrast.groups.noFoodType[i], " vs ", temp.df$contrast.groups.noFoodType[i + 1], sep = "")
  colnames(vol.intake.nanoliter.escalc) <- c("average.total.vol.intake.nanoliter.g", 
                                             "average.total.vol.intake.nanoliter.var", 
                                             "average.total.vol.intake.nanoliter.sei", 
                                             "average.total.vol.intake.nanoliter.zi", 
  ...
  vol.per.feed.per.fly.nanoliter.escalc <- escalc(data = temp.df, measure = "SMD", vtype = "UB", 
                                                  ...
                                                  sd2i = sd.vol.per.feed.per.fly.nanoliter[i], append = FALSE)
  vol.per.feed.per.fly.nanoliter.escalc <- summary.escalc(vol.per.feed.per.fly.nanoliter.escalc)
  vol.per.feed.per.fly.nanoliter.escalc$comparison <- paste(temp.df$contrast.groups.noFoodType[i], " vs ", temp.df$contrast.groups.noFoodType[i + 1], sep = "")
  colnames(vol.per.feed.per.fly.nanoliter.escalc) <- c("average.vol.per.feed.per.fly.nanoliter.g", 
                                                       "average.vol.per.feed.per.fly.nanoliter.var", 
                                                       "average.vol.per.feed.per.fly.nanoliter.sei", 
                                                       "average.vol.per.feed.per.fly.nanoliter.zi", 
  ...
  vol.per.feed.nanoliter.escalc <- escalc(data = temp.df, measure = "SMD", vtype = "UB", 
                                                  m1i = mean.vol.per.feed.nanoliter[i + 1], 
                                                  m2i = mean.vol.per.feed.nanoliter[i], 
                                                  n1i = sum.feed.count[i + 1], # NOTE THE CHANGE FROM FLY COUNT TO FEED COUNT.
                                                  n2i = sum.feed.count[i], # NOTE THE CHANGE FROM FLY COUNT TO FEED COUNT.
                                                  sd1i = sd.vol.per.feed.nanoliter[i + 1], 
                                                  sd2i = sd.vol.per.feed.nanoliter[i], append = FALSE)
  vol.per.feed.nanoliter.escalc <- summary.escalc(vol.per.feed.nanoliter.escalc)
  vol.per.feed.nanoliter.escalc$comparison <- paste(temp.df$contrast.groups.noFoodType[i], " vs ", temp.df$contrast.groups.noFoodType[i + 1], sep = "")
  colnames(vol.per.feed.nanoliter.escalc) <- c("average.vol.per.feed.nanoliter.g", 
                                                       "average.vol.per.feed.nanoliter.var", 
                                                       "average.vol.per.feed.nanoliter.sei", 
                                                       "average.vol.per.feed.nanoliter.zi", 
  ...
  mean.feeding.speed.per.fly.nl.per.s.escalc <- escalc(data = temp.df, measure = "SMD", vtype = "UB", 
                                                       ...
                                                       sd2i = sd.feeding.speed.per.fly.nl.per.s[i], append = FALSE)
  mean.feeding.speed.per.fly.nl.per.s.escalc <- summary.escalc(mean.feeding.speed.per.fly.nl.per.s.escalc)
  mean.feeding.speed.per.fly.nl.per.s.escalc$comparison <- paste(temp.df$contrast.groups.noFoodType[i], " vs ", temp.df$contrast.groups.noFoodType[i + 1], sep = "")
  colnames(mean.feeding.speed.per.fly.nl.per.s.escalc) <- c("average.feeding.speed.per.fly.nl.per.s.g", 
                                                            "average.feeding.speed.per.fly.nl.per.s.var", 
                                                            "average.feeding.speed.per.fly.nl.per.s.sei", 
                                                            "average.feeding.speed.per.fly.nl.per.s.zi", 
  ...
  proportion.of.flies.feeding.escalc <- escalc(data = temp.df, measure = "RR", 
                                               ...
                                               append = FALSE)
  proportion.of.flies.feeding.escalc <- summary.escalc(proportion.of.flies.feeding.escalc)
  proportion.of.flies.feeding.escalc$comparison <- paste(temp.df$contrast.groups.noFoodType[i], " vs ", temp.df$contrast.groups.noFoodType[i + 1], sep = "")
  colnames(proportion.of.flies.feeding.escalc) <- c("proportion.of.flies.feeding.rr", 
                                                    "log.proportion.of.flies.feeding.rr.var", 
                                                    "log.proportion.of.flies.feeding.rr.sei", 
                                                    "log.proportion.of.flies.feeding.rr.zi", 
  ...
  feed.duration.s.escalc <- summary.escalc(feed.duration.s.escalc)
  ## Note the underscores for `unite`!
  feed.duration.s.escalc$reference.contrast.group <- unite_(temp.df[i,], "reference.contrast.group", contrast.columns.noFoodType, sep = "\n")[, "reference.contrast.group"]
  feed.duration.s.escalc$tested.contrast.group <- unite_(temp.df[i + 1,], "tested.contrast.group", contrast.columns.noFoodType, sep = "\n")[, "tested.contrast.group"]
  feed.duration.s.escalc$comparison <- paste(temp.df$contrast.groups.noFoodType[i], " vs ", temp.df$contrast.groups.noFoodType[i + 1], sep = "")
  ...
  summary.escalc.noFood[[paste(x$comparison)]] <- x[,c(7:15, 1:6, 16:length(x))]
}
...
# Make sure contrast.group columns are factors with appropriate factor levels and order.
for (c in c("reference.contrast.group", "tested.contrast.group") ) {
  hedges.g.noFood[, c] <- factor( hedges.g.noFood[, c],
                                  levels = unique(reflevels.combis$contrast.groups.noFoodType) )
}

# Make sure columns that should be numeric, are.
for (colnum in 3:ncol(hedges.g.noFood)) {
  hedges.g.noFood[, colnum] <- as.numeric( as.character(hedges.g.noFood[, colnum]) )
}
...
# Write to file.
write.csv(hedges.g.noFood, file = paste(output.path, "/hedges.g.noFood.csv", sep = ""), row.names = FALSE)
```
...
```{r AveragePiPerContrastGroupTimeCourse}
if ( length(levels(rawdata.all[, "FoodIdx"])) > 1 ) {
  n <- paste("bin.", as.character(bw/60), ".min", sep = "")
  cols <- unique(c(n, contrast.columns.noFoodType)) 
  dots <- lapply(cols, as.symbol)
...
```{r DataSummary, fig.height = 25, fig.width = 18, fig.fullwidth = TRUE, results = "asis"}
...
if ( length(no.feed.flies) > 0 ) {
  ...
  f <- metadata.all[as.numeric(no.feed.flies), c(contrast.columns.noFoodType, "FlyID", "original.FlyID.feedlog", "original.feedlog.file")]
  grid.arrange(tableGrob(f),top = textGrob("Flies that didn't feed",
...
```{r RasterPlotFeedEvent, fig.height = 25, fig.width = 18, fig.fullwidth = TRUE, results = "hide"}
...
for ( c in levels(rawdata.all[, "contrast.groups.noFoodType"]) ) {
  ...
  a <- ggplot(data = droplevels(filter(rawdata.all, contrast.groups.noFoodType == c)),
    ...
    geom_bar(stat ="identity", aes(colour = FoodType, 
                                         fill = FoodType)) +
...
a <- plot_grid(plotlist = plot.list, 
          ncol = 2, rel_heights = c( rep(6, ceiling( (length(plot.list) - 2) / 2)),
                                    1) )
ggdraw(add_sub(a, "Raster Plots. A. Individual Raster Plots of Feed Events for each fly. The height of each Feed Event is proportional to the Feed Volume of that Feed.\nThe ID for each fly is labelled on the right. B. Summed Raster Plots of feeds (by food type) for all flies of that genotype."))
```
...
```{r proportionFliesFeedingStackedBar_TimeCourse, fig.height = 12, fig.width = 15, fig.fullwidth = TRUE}
...
ggdraw(
  add_sub( (ggdraw() + 
          ...
          label = "A. The proportion of flies that fed at least once across the entire assay duration is shown, for each contrast group.\nThe 95% CI for the proportion feeding is indicated by the horizontal line.\nB. The time course for how the proportion of flies feeding changes across the assay duration is shown, for each contrast group.\nEach dot represents the proportion at a specific time bin, with 95% CIs depicted by the surrounding coloured envelope. " )
)
...
```{r CumSumDataPlot, fig.height = 20, fig.width = 15, fig.fullwidth = FALSE}
...
```{r CorrelationFeedDurationFeedVolume, eval = TRUE, fig.height = 18, fig.width = 15, fig.fullwidth = TRUE}
corr <- data.frame()
...
```{r CorrelationAverageFeedSizePerFly_TotalFeedCountPerFly, eval = TRUE, fig.height = 18, fig.width = 15, fig.fullwidth = TRUE}
corr <- data.frame()
...
```{r Preference_Index_Plot, fig.height = 25, fig.width = 18, fig.fullwidth = TRUE, results = "hide"}
...
ggdraw(
  add_sub(
    ...
    label = paste("Average Preference Indices for", reflevels[["FoodType"]][2] ,". 
...
```{r TimeCourse, fig.height = 25, fig.width = 18, fig.fullwidth = TRUE, results = "hide"}
n <- paste("bin.", as.character(bw/60), ".min", sep = "")
...
#     xlab(NULL) + ylab(NULL)
  ...
  ggdraw(
    add_sub(
      x, label = paste("Time Course for ", m, ". The dots indicate the average in that time bin. 
...
```{r KruskalDunnTestTableGeneration, results = "hide"}
table.noFood <- list()
table.withFood <- list()
...
temp.rawdata <- filter(rawdata.all, !is.na(StartTime))
plot.properties[["Average Feed Size (nl)"]] <- c("Volume.mm3", # For this one, see temp.rawdata.all instead.
                                                 "average.vol.per.feed.nanoliter.g",
                                                 "average.vol.per.feed.nanoliter.ci.lb", 
                                                 "average.vol.per.feed.nanoliter.ci.ub") 
...
for (m in names(plot.properties)) {
  if ( length( levels(summary.per.fly[, "contrast.groups.noFoodType"]) ) > 1 ) {
    ...
    # Check to see if multiple corrections needs to be performed for Dunn's test.
    if ( length( levels(summary.per.fly[, "contrast.groups.noFoodType"]) ) == 2 ) {
      adj.method = "none"
    } else {
      adj.method = "bh"
    }
    ...
    if (m == "Average Feed Size (nl)") {
      a <- kruskal.test(temp.rawdata[, plot.properties[[m]][1] ], 
                      temp.rawdata[, "contrast.groups.noFoodType"])
    } else {
    ...
    if (m == "Average Feed Size (nl)") {
      d <- dunn.test(temp.rawdata[, plot.properties[[m]][1] ], 
                      temp.rawdata[, "contrast.groups.noFoodType"], method = adj.method)
    } else {
      d <- dunn.test(summary.per.fly[, plot.properties[[m]][1] ], 
                     summary.per.fly[, "contrast.groups.noFoodType"], method = adj.method)
    }
    ...
    d <- separate_(d, "Comparison", 
                   ...
                   sep = " - ")
    d[, "contrast.group.1"] <- factor(d[, "contrast.group.1"], levels = unique(reflevels.combis$contrast.groups.noFoodType))
    d[, "contrast.group.2"] <- factor(d[, "contrast.group.2"], levels = unique(reflevels.combis$contrast.groups.noFoodType))
    d[, "Adjusted\np-value"] <- as.numeric( as.character(d[, "Adjusted\np-value"]) )
    
    # Hedges' g table NO FOOD
    g <- hedges.g.noFood[ ,c("reference.contrast.group", 
                             "tested.contrast.group", 
                             plot.properties[[m]][2],
                             plot.properties[[m]][3],
                             plot.properties[[m]][4]) ]
    ...
    table.noFood[[m]] <- left_join(g, d, by = c("reference.contrast.group" = "contrast.group.1", 
                                                "tested.contrast.group" = "contrast.group.2"))
    # Join them together, part 2
    table.noFood[[m]] <- left_join(table.noFood[[m]], d, by = c("reference.contrast.group" = "contrast.group.2", 
                                                                "tested.contrast.group" = "contrast.group.1"))
    table.noFood[[m]] <- unite_(table.noFood[[m]], "Adjusted\np-value", 
                                c("Adjusted\np-value.x", "Adjusted\np-value.y"),
                                sep = "")
    # format the column properly
    table.noFood[[m]][, "Adjusted\np-value"] <- gsub("\\NA", "\\", table.noFood[[m]][, "Adjusted\np-value"])
    table.noFood[[m]][, "Adjusted\np-value"] <- as.numeric( as.character(table.noFood[[m]][, "Adjusted\np-value"]) )

    colnames(table.noFood[[m]]) <- c("Reference\ngroup",
                                     "Tested\ngroup",
                                     "Hedges' g\n[95% CI]",
    ...
    table.noFood[[m]] <- gtable_add_rows(table.noFood[[m]], 
                                         ...
                                         pos = 0)
    table.noFood[[m]] <- gtable_add_grob(table.noFood[[m]], list(kw.line.noFood),
                                         t = 1, l = 1, r = ncol(table.noFood[[m]]))
    ...
    if ( length(levels(rawdata.all[, "FoodIdx"])) > 1 ) {
      ...
      # Check to see if multiple corrections needs to be performed for Dunn's test.
      if ( length( levels(summary.per.fly[, "contrast.groups.withFoodType"]) ) == 2 ) {
        adj.method = "none"
      } else {
        adj.method = "bh"
      }
      ...
      if (m == "Average Feed Size (nl)") {
        a <- kruskal.test(temp.rawdata[, plot.properties[[m]][1] ], 
                          temp.rawdata[, "contrast.groups.withFoodType"])
      } else {
              a <- kruskal.test(summary.per.fly[, plot.properties[[m]][1] ], 
      ...
      if (m == "Average Feed Size (nl)") {
        d <- dunn.test(temp.rawdata[, plot.properties[[m]][1] ], 
                       temp.rawdata[, "contrast.groups.withFoodType"], method = adj.method)
      } else {
        d <- dunn.test(summary.per.fly[, plot.properties[[m]][1] ], 
                       summary.per.fly[, "contrast.groups.withFoodType"], method = adj.method)
      }
      ...
      d <- separate_(d,  "Comparison", 
                     c("contrast.group.1", "contrast.group.2"),
                     sep = " - ")
      d[, "contrast.group.1"] <- factor(d[, "contrast.group.1"], levels = unique(reflevels.combis$contrast.groups.withFoodType))    
      d[, "contrast.group.2"] <- factor(d[, "contrast.group.2"], levels = unique(reflevels.combis$contrast.groups.withFoodType))    
      
      # Hedges' g table WITH FOOD
      g <- hedges.g.withFood[ ,c("reference.contrast.group", 
                                 "tested.contrast.group", 
                                 plot.properties[[m]][2],
                                 plot.properties[[m]][3],
                                 plot.properties[[m]][4]) ]
      ...
      # Join them together, part 1
      table.withFood[[m]] <- left_join(g, d, by = c("reference.contrast.group" = "contrast.group.1", 
                                                    "tested.contrast.group" = "contrast.group.2"))
      # Join them together, part 2
      table.withFood[[m]] <- left_join(table.withFood[[m]], d, by = c("reference.contrast.group" = "contrast.group.2", 
                                                                      "tested.contrast.group" = "contrast.group.1"))
      table.withFood[[m]] <- unite_(table.withFood[[m]], "Adjusted\np-value", 
                                    c("Adjusted\np-value.x", "Adjusted\np-value.y"),
                                    sep = "")
      # format the column properly
      table.withFood[[m]][, "Adjusted\np-value"] <- gsub("\\NA", "\\", table.withFood[[m]][, "Adjusted\np-value"])
      table.withFood[[m]][, "Adjusted\np-value"] <- as.numeric( as.character(table.withFood[[m]][, "Adjusted\np-value"]) )

      colnames(table.withFood[[m]]) <- c("Reference\ngroup",
                                         "Tested\ngroup",
                                         "Hedges' g\n[95% CI]",
                                         "Post-hoc Dunn's Test\nAdjusted p-value")
...
```{r DensityDistributionFeedCount, fig.height = 16, fig.width = 10, fig.fullwidth = TRUE}
plot.list <- list()

if ("FoodType" %in% contrast.columns) {
  # Density Distrib with Food Type
  ...
  # Density Distrib without Food Type
  b <- ggplot(data = summary.per.fly, 
              aes(x = total.feed.count)) +
    geom_rug(alpha = 0.3) + # geom_histogram(binwidth = 1) +
    geom_density(adjust = 2, alpha = 0.5, linetype = "dotted", fill = "lightgrey") + 
  ...
  # See http://stackoverflow.com/questions/7705345/how-can-i-extract-plot-axes-ranges-for-a-ggplot2-object for extracting limits!
  xrange <- ggplot_build(b)$panel$ranges[[2]]$x.range
  yrange <- ggplot_build(b)$panel$ranges[[2]]$y.range
  
  plot.list[[2]] <- b + geom_text(data = summary.per.contrastGroup.noFood,
                                  aes(label = paste("mean = ", mean.total.feed.count.per.fly, sep = ""),
  ...
  plot.list[[1]] <- a + geom_text(data = summary.per.contrastGroup.noFood,
                                  aes(label = paste("mean = ", mean.total.feed.count.per.fly, sep = ""),
                                      x = mean.total.feed.count.per.fly,
                                      y = yrange[2]), 
                                  colour = "orange",
...
if (length(plot.list) == 1) {
  ggdraw(
    add_sub(plot.list[[1]], label = "Density Distribution of Total Feed Counts per Fly. The orange line indicates the mean of the distribution, while the purple line indicates the median.")
  )
} else {
  ggdraw(
    add_sub(plot_grid(plotlist = plot.list, ncol = 1, nrow = 2), label = "Density Distribution of Total Feed Counts per Fly. The orange line indicates the mean of the distribution, while the purple line indicates the median.")
  )
}
```

```{r feedCountSummaryPlot_and_Table, fig.height = 18, fig.width = 15, fig.fullwidth = TRUE, fig.cap = ""}
...
#### Make line-range plot No Food
  ...
  for (x in 1: nrow(temp.df)) {
    temp.df[x, "Average\ntotal feed count\nper fly [95% CI]"] <- paste(temp.df[x, "Average\ntotal feed count\nper fly [95% CI]"], 
                                                                       " [", temp.df[x, "low"], 
                                                                       "; ", temp.df[x, "high"],
                                                                       "]", sep = "")
  }
...
```{r feedCountKruskalDunnTestHedgesG, results = "hide", fig.height = 25, fig.width = 20, fig.fullwidth = TRUE}
if ( length(levels(rawdata.all[, "FoodIdx"])) > 1 ) {
  
  grid.arrange(table.noFood[["Feed Count"]])
  grid.arrange(table.withFood[["Feed Count"]])
...
```{r DensityDistributionTotalVolumeIntake, fig.height = 16, fig.width = 10, fig.fullwidth = TRUE}
plot.list <- list()
...
if (length(plot.list) == 1) {
  ggdraw(
    add_sub(plot.list[[1]], label = "Density Distribution of Total Volume Intake. The orange line indicates the mean of the distribution, while the purple line indicates the median.")
  )
} else {
  ggdraw(
    add_sub(plot_grid(plotlist = plot.list, ncol = 1, nrow = 2), label = "Density Distribution of Total Volume Intake. The orange line indicates the mean of the distribution, while the purple line indicates the median.")
  )
}
```

```{r VolumeIntakeSummaryPlot_And_Table, fig.height = 18, fig.width = 15, fig.fullwidth = TRUE}
...
#### Make line-range plot No Food
  summ.plot.noFood <- ggplot() + 
    ...
    guides( alpha = FALSE, size = FALSE, fill = FALSE, colour = FALSE, shape = FALSE) +
    scale_y_continuous(
#       breaks = seq(from = 0, to = ylim[2], by = 20),
#       labels = seq(from = 0, to = ylim[2], by = 20),
      expand = c(0.01, 0)) + # multiplicative expansion factor so zero feeds show up nicely
    ylab(ylab) + xlab("") + 
    theme_minimal() + #ggtitle(plot.title) +
  ...
  for (x in 1: nrow(temp.df)) {
    temp.df[x, "Mean total\nVol. intake (nl)\n[95% CI]"] <- paste(temp.df[x, "Mean total\nVol. intake (nl)\n[95% CI]"], 
                                                                  " [", temp.df[x, "low"], 
                                                                  "; ", temp.df[x, "high"],
                                                                  "]", sep = "")
  }
...
```{r VolumeIntakeKruskalDunnTestHedgesG, fig.height = 25,  fig.width = 20, fig.fullwidth = TRUE}
if ( length(levels(rawdata.all[, "FoodIdx"])) > 1 ) {

  grid.arrange(table.noFood[["Total Volume Intake (nl)"]])
  grid.arrange(table.withFood[["Total Volume Intake (nl)"]])
...
```{r DensityDistributionTimeToFirstFeed, fig.height = 16, fig.width = 10, fig.fullwidth = TRUE}
plot.list <- list()
...
if (length(plot.list) == 1) {
  ggdraw(
    add_sub(plot.list[[1]], label = "Density Distribution of Time to First Feed. The orange line indicates the mean of the distribution, while the purple line indicates the median.")
  )
} else {
  ggdraw(
    add_sub(plot_grid(plotlist = plot.list, ncol = 1, nrow = 2), label = "Density Distribution of Time to First Feed. The orange line indicates the mean of the distribution, while the purple line indicates the median.")
  )
}
```

```{r TimeToFirstFeedSummaryPlot_and_Table, fig.height = 18, fig.width = 15, fig.fullwidth = TRUE}
...
#### Make line-range plot No Food
  ...
  for (x in 1: nrow(temp.df)) {
    temp.df[x, "mean\ntime to\nfirst feed (min) [95% CI]"] <- paste(temp.df[x, "mean\ntime to\nfirst feed (min) [95% CI]"], 
                                                                    " [", temp.df[x, "low"], 
                                                                    "; ", temp.df[x, "high"],
                                                                    "]", sep = "")
  }
...
```{r TimeToFirstFeedKruskalDunnTestHedgesG, fig.height = 25,  fig.width = 20, fig.fullwidth = TRUE}
if ( length(levels(rawdata.all[, "FoodIdx"])) > 1 ) {
  
  grid.arrange(table.noFood[["Time To First Feed (s)"]])
  grid.arrange(table.withFood[["Time To First Feed (s)"]])
...
## Feed Size (aka Meal Size) Summary

The section calculates averages in two different ways: at the level of individual flies, and at the level of contrast groups. For more information see this [Google spreadsheet](https://docs.google.com/spreadsheets/d/19mGXNM_dSc_Y3y5jy4Zm8LFybiSITK4EZ-GP9L-vAJY/edit#gid=0)

```{r DensityDistributionFeedSizePerFly, fig.height = 16, fig.width = 10, fig.fullwidth = TRUE}
plot.list <- list()
...
if (length(plot.list) == 1) {
  ggdraw(
    add_sub(plot.list[[1]], label = "Density Distribution of Feed Size per Fly. The orange line indicates the mean of the distribution, while the purple line indicates the median.")
  )
} else {
  ggdraw(
    add_sub(plot_grid(plotlist = plot.list, ncol = 1, nrow = 2), label = "Density Distribution of Feed Size per Fly. The orange line indicates the mean of the distribution, while the purple line indicates the median.")
  )
}
```

```{r FeedSizePerFlySummaryPlot_And_Table, fig.height = 18, fig.width = 15, fig.fullwidth = TRUE}
...
#       labels = seq(from = 0, to = ylim[2], by = 3),
  ...
  for (x in 1: nrow(temp.df)) {
    temp.df[x, "mean \nfeed size\nper feeding fly (nl)"] <- paste(temp.df[x, "mean \nfeed size\nper feeding fly (nl)"], 
                                                                  " [", temp.df[x, "low"], 
                                                                  "; ", temp.df[x, "high"],
                                                                  "]", sep = "")
  }
...
```{r FeedSizePerFlyKruskalDunnTestHedgesG, fig.height = 25, fig.width = 20, fig.fullwidth = TRUE}
if ( length(levels(rawdata.all[, "FoodIdx"])) > 1 ) {

  grid.arrange(table.noFood[["Feed Size per fly (nl)"]])
  grid.arrange(table.withFood[["Feed Size per fly (nl)"]])
  
  } else if ( length(levels(rawdata.all[, "FoodIdx"])) == 1 ) {
    grid.arrange(table.noFood[["Feed Size per fly (nl)"]],
                 top = textGrob("Average Feed Size Per Fly\nPairwise Comparisons",
                                gp = gpar(fontface = "bold", 
                                          fontsize = 20),
                                vjust = 1) )
  } else if ( length(levels(summary.per.fly[, "contrast.groups.noFoodType"])) == 1 )  {
...
```{r FeedSizeSummaryPlot_And_Table, fig.height = 18, fig.width = 15, fig.fullwidth = TRUE}
a <- "mean.vol.per.feed.nanoliter" # from summary.per.fly
b <- "mean.vol.per.feed.nanoliter" # from summary.per.contrastGroup.withFood
c <- "ci.lower.vol.per.feed.nanoliter"
d <- "ci.upper.vol.per.feed.nanoliter"
ylab <- "Average Feed Size (nl)"
plot.title <- "Average Feed Size\nSummary Plot"
bin.width <- 0.25
dot.size <- 6
...
#### Figure Label
fig.label <- data.frame(label = "Average Feed Size Summary Plot. Each scatterpoint is the average feed size for a single fly. The vertical crossbar 
indicates the average feed size (calculated at the level of the group, not the fly) for that contrast group; the horizontal crossbar 
...
#       labels = seq(from = 0, to = ylim[2], by = 3),
  ...
  temp.df <- summary.per.contrastGroup.noFood[, c(contrast.columns.noFoodType, "total.fly.count", "feeding.fly.count", 
                                                  "mean.vol.per.feed.nanoliter", 
                                                  "median.vol.per.feed.nanoliter", "sd.vol.per.feed.nanoliter", 
                                                  "ci.lower.vol.per.feed.nanoliter","ci.upper.vol.per.feed.nanoliter")]
  ...
  for (x in 1: nrow(temp.df)) {
    temp.df[x, "mean \nfeed size (nl)"] <- paste(temp.df[x, "mean \nfeed size (nl)"], 
                                                                  " [", temp.df[x, "low"], 
                                                                  "; ", temp.df[x, "high"],
                                                                  "]", sep = "")
  }
...
```{r FeedSizeKruskalDunnTestHedgesG, fig.height = 25, fig.width = 20, fig.fullwidth = TRUE}
if ( length(levels(rawdata.all[, "FoodIdx"])) > 1 ) {
  
  grid.arrange(table.noFood[["Average Feed Size (nl)"]])
  grid.arrange(table.withFood[["Average Feed Size (nl)"]])  
...
```{r DensityDistributionFeedDuration, fig.height = 16, fig.width = 10, fig.fullwidth = TRUE}
plot.list <- list()
...
if (length(plot.list) == 1) {
  ggdraw(
    add_sub(plot.list[[1]], label = "Density Distribution of Feed Duration. The orange line indicates the mean of the distribution, while the purple line indicates the median.")
  )
} else {
  ggdraw(
    add_sub(plot_grid(plotlist = plot.list, ncol = 1, nrow = 2), label = "Density Distribution of Feed Duration. The orange line indicates the mean of the distribution, while the purple line indicates the median.")
  )
}
```

```{r feedDurationSummaryPlot_And_Table, fig.height = 18, fig.width = 15, fig.fullwidth = TRUE}
...
```{r feedDurationKruskalDunnTestHedgesG, fig.height = 25, fig.width = 20, fig.fullwidth = TRUE}
if ( length(levels(rawdata.all[, "FoodIdx"])) > 1 ) {
   
  grid.arrange(table.noFood[["Feed Duration (s)"]])
  grid.arrange(table.withFood[["Feed Duration (s)"]])  
...
```{r DensityDistributionFeedingSpeed, fig.height = 16, fig.width = 10, fig.fullwidth = TRUE}
plot.list <- list()
...
if (length(plot.list) == 1) {
  ggdraw(
    add_sub(plot.list[[1]], label =  "Density Distribution of Feeding Speed. The orange line indicates the mean of the distribution, while the purple line indicates the median.")
  )
} else {
  ggdraw(
    add_sub(plot_grid(plotlist = plot.list, ncol = 1, nrow = 2), label =  "Density Distribution of Feeding Speed. The orange line indicates the mean of the distribution, while the purple line indicates the median.")
  )
}
```

```{r feedingSpeedSummaryPlot_And_Table, fig.height = 18, fig.width = 15, fig.fullwidth = TRUE}
#### Make Summary Table
...
```{r feedingSpeedKruskalDunnTestHedgesG, fig.height = 25,  fig.width = 20, fig.fullwidth = TRUE}
if ( length(levels(rawdata.all[, "FoodIdx"])) > 1 ) {
  
  grid.arrange(table.noFood[["Feeding Speed (nl/s)"]])
  grid.arrange(table.withFood[["Feeding Speed (nl/s)"]])  
...
```{r Comparison, eval = FALSE, fig.height = 20, fig.width = 18, fig.fullwidth = TRUE}

#####
# This chunk is currently not run. I have to figure out a elegant way to display the effect sizes...
#####
plot.properties <- list()
